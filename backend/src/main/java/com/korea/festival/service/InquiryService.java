...
public class InquiryService {
	...
	public Page<InquiryDto> getUserInquiries(String username, Pageable pageable) {
		...
		return inquiryRepository.findByUserOrderByCreatedAtDesc(user, pageable).map(this::convertToDTO);
	}
	...
	private InquiryDto convertToDTO(Inquiry inquiry) {
		return new InquiryDto(inquiry.getId(), inquiry.getTitle(), inquiry.getContent(), inquiry.getStatus().toString(),
				inquiry.getAnswer(), inquiry.getCreatedAt(), inquiry.getAnsweredAt());
	}
}